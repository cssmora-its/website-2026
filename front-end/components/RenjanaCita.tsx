...
export default function RenjanaCita() {
  // Daftar nama file 7 departemen sesuai dengan yang kamu upload di folder public
  // Pastikan penulisan huruf besar/kecil dan ekstensinya (.png) sama persis!
  ...
  // State untuk melacak urutan foto yang sedang aktif (mulai dari indeks 0)
  const [currentIndex, setCurrentIndex] = useState(0);

  // Fungsi untuk menggeser ke foto sebelumnya
  const handlePrev = () => {
    setCurrentIndex((prev) => (prev === 0 ? departemenImages.length - 1 : prev - 1));
  };

  // Fungsi untuk menggeser ke foto selanjutnya
  const handleNext = () => {
      ...
      <div className="container mx-auto px-6 max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-12 items-center">
        ...
        <div className="flex flex-col w-full max-w-[550px] mx-auto lg:ml-auto mt-6 lg:mt-0">
           ...
           <div className="flex items-center justify-between w-full mt-6 text-white">
             ...
             {/* Indikator Angka (Dibuat Dinamis) */}
             ...
             {/* Tombol Navigasi Panah (Diberi event onClick) */}
             <div className="flex gap-4 md:gap-6">
               <button onClick={handlePrev} className="hover:opacity-75 transition-opacity" aria-label="Previous">
               ...
               <button onClick={handleNext} className="hover:opacity-75 transition-opacity" aria-label="Next">
                 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-7 h-7 md:w-8 md:h-8">
